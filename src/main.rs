...
fn stack_loop_n(n: u32) -> Duration {
    let q = Stack::new();
...
fn stack_thread_n_m(n: u32, m: u32) -> Duration {
    let mut handles = Vec::new();
    let elapsed = Arc::new(AtomicU64::default());
...
fn main() {
    // loop operation.
    let m = 100000;
    let mut dur = stack_loop_n(m);
    println!("stack {} ops: {:?}, avg: {:?}", m, dur, dur.checked_div(m));
